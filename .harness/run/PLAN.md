...
| 3 | T-004, T-007 | `res/values/strings.xml`/`values-de/strings.xml` (edit-dialog strings), `app/src/test/.../todo/FakeTaskRepository.kt` (`updateTitle`, mirroring T-004's interface), `app/src/test/.../calendar/FakeNoteRepository.kt` (`update`/`delete`, mirroring T-007's interface), `ui/fragment/todo/component/TodoTaskItem.kt` + `ui/fragment/todo/TodoFragment.kt` (edit entry point + dialog wiring, out of T-004's scope), `ui/fragment/calendar/CalendarFragment.kt` (edit/delete callback + dialog wiring, out of T-007's scope) |
